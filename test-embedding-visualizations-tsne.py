...
filename = '/Users/dwaste/Desktop/Undergrad-Thesis-Repo/repo_glove2word2vec_model.sav'
...
input_word = 'ucrânia, antiglobalización, antirrussa'
...
def display_tsne_scatterplot_2D(model, user_input=None, words=None, label=None, color_map=None, perplexity = 0, learning_rate = 0, iteration = 0, topn=5, sample=10):
    ...
    two_dim = TSNE(n_components = 2, random_state=0, perplexity = perplexity, learning_rate = learning_rate, n_iter = iteration).fit_transform(word_vectors)[:,:3]
    # For 2D, change the three_dim variable into something like two_dim like the following:
    # two_dim = TSNE(n_components = 2, random_state=0, perplexity = perplexity, learning_rate = learning_rate, n_iter = iteration).fit_transform(word_vectors)[:,:2]
...
display_tsne_scatterplot_2D(model, user_input, similar_word, labels, color_map, 5, 500, 10000)
