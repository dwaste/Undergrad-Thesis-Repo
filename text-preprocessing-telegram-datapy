...
# drop rows with missing values
df["formatted_text"] = df["formatted_text"].dropna()

# write the DataFrame to a new CSV file need to have header=True, when appending header=False
df.to_csv('/Users/dwaste/Desktop/Undergrad-Thesis-Repo/transformed-data/telegram-transformed-data.csv', mode = 'w', index=False, header=True, encoding='utf-8')
